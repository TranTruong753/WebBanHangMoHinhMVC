...
" on sanpham.MaSanPham=chitietsanpham.MaSanPham Where chitiethoadon.MaHoaDon='"+mahd+"'";
  ...
  loadPhanTrang("chitiethoadon", index, size, sql, "");
...
function loadTable(key, index, size) {
  $.ajax({
    url: "http://localhost/WebBanHangMoHinhMVC/AjaxChiTietHoaDon/getDanhSach",
    type: "post",
    dataType: "html",
    data: {
      key: tmpKey,
...
$(document).on("click", ".btnPhanTrang", function() {
  ...
  // alert(tmpKey)
  loadPhanTrang("chitiethoadon", index, size, sql, tmpKey);
})
...
$(document).on("click", "#btnSearch", function() {
  ...
  // xử lý số trang đã chọn
  loadPhanTrang("chitiethoadon", index, size, sql, key);
...
loadPhanTrang("chitiethoadon", index, size, "", key);
})
